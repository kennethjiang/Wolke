        ...
        public function StartThread($bundle_task_info)
        {
        	...
        	Scalr::ReconfigureObservers();
         	...
         	$BundleTask = BundleTask::LoadById($bundle_task_info['id']);
                ...
                $this->Logger->info("Start thread: ".$BundleTask->serverId);
        	try
         	{
         	...
         	catch (Exception $e)
         	{
                ...
                $this->Logger->info("bundle task sttaus: " . $BundleTask->status);
         	...
         	switch($BundleTask->status)
         	{
         		...
         		case SERVER_SNAPSHOT_CREATION_STATUS::REPLACING_SERVERS:
         			...
         			{
	         			if ($DBFarmRole->GetSetting(DBFarmRole::SETTING_CLOUD_LOCATION) != $BundleTask->cloudLocation) {
         		...
         		case SERVER_SNAPSHOT_CREATION_STATUS::CREATING_ROLE:
         			...
         			try
         			{
         				if ($BundleTask->replaceType == SERVER_REPLACEMENT_TYPE::REPLACE_ALL)
         				{
	         				...
	         				elseif (is_array($dbRole))
	         				{
	         					$this->Logger->error("dbRole->replace->array({$BundleTask->roleName}, {$BundleTask->envId})");
	         				}
         				}
         				...
         				{
         					$BundleTask->SnapshotCreationFailed("Role creation failed due to internal error. Please try again.");
         					return;
         				}
