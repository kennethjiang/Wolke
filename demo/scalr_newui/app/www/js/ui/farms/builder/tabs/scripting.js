	...
	return new Scalr.ui.farms.builder.tab({
		...
		activateTab: function () {
			...
			this.findOne('itemId', 'scripting.all').on('selectionchange', function (dataview, selections) {
				...
				if (selections.length) {
					...
					this.findOne('name', 'scripting.edit.when').setValue(
						toolbar.getComponent('event').store.getById(rec.get('event')).get('name')
					);
			...
			this.findOne('name', 'scripting.edit.version').on('select', function (field, record) {
				var fields = record.get('fields') || '', fieldset = this.findOne('itemId', 'scripting.edit.parameters');
			...
			if (! this.loaded) {
				Scalr.Request({
					...
					success: function (data) {
						toolbar.getComponent('script').store.loadData(data.scripts);
						toolbar.getComponent('event').store.loadData(data.events);
