...
var trackbar = { // NAMESPACE
	archive : {},
	getObject : function(id, force) {
		if (typeof this.archive[id] == "undefined" || force == true) {
...
// Methods
	...
	*/
	...
	init : function(hash, node) {
		...
		try {
			...
			var code = '<div class="tickdiv" id="'+this.TICKDIV_PREFIX+this.id+'"></div><table id="'+this.TABLE_PREFIX+this.id+'" ' + (this.width ? ' style="width:'+this.width+'px;"' : '') + 'class="trackbar" onSelectStart="return false;">\
				<tr>\
					<td class="l"><div id="leftBlock_' + this.id + '"><span></span><span class="limit"></span><img id="leftBegun_' + this.id + '" ondragstart="return false;" src="/js/FC_TrackBar/imgtrackbar/b_l.gif" width="5" height="17" alt="" style="vertical-align:top;" /></div></td>\
					<td class="c" id="centerBlock_' + this.id + '"></td>\
					<td class="r"><div id="rightBlock_' + this.id + '"><span></span><span class="limit"></span><img id="rightBegun_' + this.id + '" ondragstart="return false;" src="/js/FC_TrackBar/imgtrackbar/b_r.gif" width="5" height="17" alt="" style="vertical-align:top;" /></div></td>\
				</tr>\
			</table>';
			...
			this.rightValue = (this.rightValue || this.rightLimit).toFixed(1);
			this.leftValue = (this.leftValue || this.leftLimit).toFixed(1);
			...
			this.leftWidth = this.countWidth(this.leftValue);
			...
			this.rightWidth = 200-this.countWidth(this.rightValue);
	...
	onMove2: function() {
		...
		Ext.get(this.CENTER_BLOCK_PREFIX+this.id).removeClass('trackbar_red');
		Ext.get(this.CENTER_BLOCK_PREFIX+this.id).removeClass('trackbar_yellow');
		Ext.get(this.CENTER_BLOCK_PREFIX+this.id).removeClass('trackbar_normal');
		
		if (delta <= 1)
		{
			Ext.get(this.CENTER_BLOCK_PREFIX+this.id).addClass('trackbar_red');
		}
		else if (delta < 2)
		{
			Ext.get(this.CENTER_BLOCK_PREFIX+this.id).addClass('trackbar_yellow');
		}
		else if (delta >= 2)
		{
			Ext.get(this.CENTER_BLOCK_PREFIX+this.id).addClass('trackbar_normal');
		}
	...
	updateRightValue : function(rightValue) {
		try {
			this.rightValue = (rightValue).toFixed(1);
	...
	rightCounter : function() {
		if (this.dual) {
			...
			width = 200-this.rightWidth;
			this.rightValue = this.countValue(width);
			
			if (this.roundUp)
			{
				this.rightValue = parseInt(this.rightValue / this.roundUp) * this.roundUp;
			}
	...
	updateLeftValue : function(leftValue) {
		try {
			this.leftValue = (leftValue).toFixed(1);
	...
	{
		var X = this.width/this.rightLimit;
		
		Y = value;
		
		return Math.sqrt((100*this.width*Y*X)/100);
	},
	
	countValue: function (width)
	{
		var P = (width == this.width-6) ? 100 : width/this.width*100

		var X = this.width/this.rightLimit;
		
		var Y = (P == 100) ? this.rightLimit : (width/X/100*P).toFixed(1);
		
		if (Y < this.leftLimit)	Y = this.leftLimit; 
	...
	leftCounter : function() {
		if (this.dual) 
		{
			...
			this.leftValue = this.countValue(this.leftWidth);
			
			if (this.roundUp)
			{
				this.leftValue = parseInt(this.leftValue / this.roundUp) * this.roundUp;
			}
