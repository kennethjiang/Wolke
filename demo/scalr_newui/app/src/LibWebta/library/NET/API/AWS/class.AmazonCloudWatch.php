<?php
	...
	class AmazonCloudWatch
	{
		...
		{
			 self::$Instance = new AmazonCloudWatch($AWSAccessKeyId, $AWSAccessKey);
		...
		{
			$response = $this->Request("GET", "/", array("Action" => "ListMetrics"));
			...
			foreach ($metrics['member'] as $item)
			{
				...
				if (!$list[(string)$item->Namespace][(string)$item->MeasureName])
					$list[(string)$item->Namespace][(string)$item->MeasureName] = array();
				
				$a = (array)$item->Dimensions->member;
				
				if (!empty($a))
				{
					if (!$list[(string)$item->Namespace][(string)$item->MeasureName][$a['Name']])
						$list[(string)$item->Namespace][(string)$item->MeasureName][$a['Name']] = array();
						
					$list[(string)$item->Namespace][(string)$item->MeasureName][$a['Name']][] = $a['Value'];
				}
			}
