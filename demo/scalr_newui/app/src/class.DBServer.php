<?php
	...
	{			
		...
		{
			if (!$this->propsCache[$propertyName] || $skipCache)
			{
		...
		public function GetFreeDeviceName()
		{
			...
			preg_match_all("/sd([a-z])[0-9]*/", $result, $matches);
			...
			{
				$device_l = $map[@mt_rand(0, @count($map)-1)];
				@array_splice($map, @array_search($device_l, $map), 1);
			}
		 ...
		 */
		public function SendMessage(Scalr_Messaging_Msg $message, $isEventNotice = false)
		{
			...
			if ($this->IsSupported("0.5") && !$isEventNotice)
			{
				...
				{
					// Prepare request
				  	$request = new HttpRequest("http://{$this->remoteIp}:8013/control", HTTP_METH_POST);
					...
					if ($request->getResponseCode() == 201) {
						...
						$logger->error(sprintf("[FarmID: %s] Cannot deliver message '%s' (message_id: %s) via REST"
