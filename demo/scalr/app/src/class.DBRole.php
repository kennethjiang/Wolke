	...
	class DBRole
	{
		...
		public function save()
		{
			if (!$this->id) {
				...
				$this->origin, $this->envId, APPROVAL_STATE::APPROVED, $this->clientId, $this->behaviorsRaw, $this->os));
		...
		public function remove($removeImage = false)
		{
			...
			$this->db->Execute("DELETE FROM role_behaviors WHERE role_id = ?", array($this->id));
			$this->db->Execute("DELETE FROM role_images WHERE role_id = ?", array($this->id));
			$this->db->Execute("DELETE FROM role_parameters WHERE role_id = ?", array($this->id));
			$this->db->Execute("DELETE FROM role_properties WHERE role_id = ?", array($this->id));
			$this->db->Execute("DELETE FROM role_security_rules WHERE role_id = ?", array($this->id));
			$this->db->Execute("DELETE FROM role_software WHERE role_id = ?", array($this->id));
		...
		{
			if (!$this->db->GetOne("SELECT id FROM role_images WHERE image_id = ? AND role_id != ?", array(trim($imageId), $this->id)))
			{
				$this->db->Execute("REPLACE INTO role_images SET
					role_id			= ?,
					cloud_location	= ?,
					image_id		= ?,
					platform		= ?
