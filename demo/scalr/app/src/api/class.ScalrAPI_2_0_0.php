	...
	class ScalrAPI_2_0_0 extends ScalrAPICore
	{
		...
		public function DNSZoneCreate($DomainName, $FarmID = null, $FarmRoleID = null)
		{
			...
			while (count($domain_chunks) > 0)
			{
				...
				if ($this->DB->GetOne("SELECT id FROM dns_zones WHERE zone_name=? AND client_id != ?", array($chk_dmn, $this->Client->ID)))
				{
					if ($chk_dmn == $DomainName)
						throw new Exception(sprintf(_("%s already exists on scalr nameservers"), $DomainName));
					else
						throw new Exception(sprintf(_("You cannot use %s domain name because top level domain %s does not belong to you"), $req_domainname, $chk_dmn));
				}
			}
			...
			$DBDNSZone = DBDNSZone::create(
				...
				14400, 
				86400,
				str_replace('@', '.', $this->Client->Email)
			);
			
			$DBDNSZone->farmRoleId = (int)$FarmRoleID;
			$DBDNSZone->farmId = (int)$FarmID;
			$DBDNSZone->clientId = $this->Client->ID;
			$DBDNSZone->envId = $this->Environment->id;
			
			$def_records = $this->DB->GetAll("SELECT * FROM default_records WHERE clientid=?", array($this->Client->ID));
		...
		public function ServerLaunch($FarmRoleID)
		{
			try
			{
				$DBFarmRole = DBFarmRole::LoadByID($FarmRoleID);
        	...
        	if ($DBFarmRole->GetRunningInstancesCount() < $min_instances)
