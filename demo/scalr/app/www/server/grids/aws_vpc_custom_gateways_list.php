<?php
	...
	try
	{
		...
		if ($req_sort)
		{
			$nrowz = array();
			foreach ($rowz as $row)				
				$nrowz[(string)$row['customerGatewayId']] = $row;			
		...
		{ 	
			$response["data"][] = array(
					"customer_id"	=> (string)$row['customerGatewayId'], // have to call only like "id" for correct script work in template
					"type"			=> (string)$row['type'],
					"state"			=> (string)$row['state'],					
					"ipAddress"		=> (string)$row['ipAddress'],
					"bgpAsn"		=> (string)$row['bgpAsn']
					);				
		}
