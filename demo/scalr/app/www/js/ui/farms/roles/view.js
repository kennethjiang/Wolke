{
	create: function (loadParams, moduleParams) {
		...
		return new Scalr.Viewers.ListView({
			title: '服务器组 &raquo; ' + moduleParams['farmName'] + ' &raquo; 服务角色',
			scalrOptions: {
				'reload': false,
				'maximize': 'all'
			},
			...
			listViewOptions: {
				...
				columns: [
					...
					{ header: "服务器", width: 20, dataIndex: 'servers', sortable: false, hidden: 'no', tpl:
						'{servers} [<a href="#/servers/view?farmId={farmid}&farmRoleId={id}">查看</a>]'
					},
					{ header: "域", width: 20, dataIndex: 'domains', sortable: false, hidden: 'no', tpl:
						'{domains} [<a href="#/dnszones/view?farmRoleId={id}">查看</a>]'
					}
			]},
			listeners:{
