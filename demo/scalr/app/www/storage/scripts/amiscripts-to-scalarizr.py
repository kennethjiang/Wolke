...
def configure_scalarizr_mysql_ebs():
	vol_conf = dict(type='ebs', id=mysql_ebs_volume_id, device='/dev/sdo')	
	...
	if os.path.exists('/mnt/mysql-data/mysql/mysql'):
		...
		for file in os.listdir('/mnt/mysql-data/mysql'):
			if file != 'mysql':
				shutil.move(os.path.join('/mnt/mysql-data/mysql', file), '/mnt/mysql-data')
		for file in os.listdir('/mnt/mysql-data/mysql/mysql'):
			shutil.move(os.path.join('/mnt/mysql-data/mysql/mysql', file), '/mnt/mysql-data/mysql')
...
def configure_scalarizr_apache():
	...
	duplicates = list('%s%s.vhost.conf' % (vhost.hostname, '-ssl' if vhost.https else '') for vhost in vhosts)
	if duplicates:
		logger.info('Removing scalr-managed virtual hosts from /etc/apache2/sites-enabled. '
				'They will be available at /etc/scalr/private.d/vhosts')
